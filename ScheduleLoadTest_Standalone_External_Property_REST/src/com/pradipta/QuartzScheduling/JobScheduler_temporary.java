...
public class JobScheduler_temporary {
    public static void main(String[] args) throws Exception {
        ...
        JobDetail job =
            JobBuilder.newJob(com.pradipta.QuartzScheduling.QuartzJobDefn.class).withDescription(configurationParameters.regionForDemo.toString()).withIdentity(configurationParameters.schedulerJobName,
                                                                                                                                                                configurationParameters.loadGenerationCategory.toString()).build();

        if (configurationParameters.timeIntervalUnit == configurationParameters.timeIntervalUnitEnum.MINUTE) {
            trigger =
                TriggerBuilder.newTrigger().withDescription(configurationParameters.regionForDemo.toString()).withIdentity(configurationParameters.schedulerJobName,
                                                                                                                           configurationParameters.loadGenerationCategory.toString()).startAt(startDate).withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInMinutes(configurationParameters.timeInterval).repeatForever()).endAt(endDate).build();
        } else if (configurationParameters.timeIntervalUnit == configurationParameters.timeIntervalUnitEnum.HOUR) {
            trigger =
                TriggerBuilder.newTrigger().withDescription(configurationParameters.regionForDemo.toString()).withIdentity(configurationParameters.schedulerJobName,
                                                                                                                           configurationParameters.loadGenerationCategory.toString()).startAt(startDate).withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInHours((configurationParameters.timeInterval)).repeatForever()).endAt(endDate).build();
        } else if (configurationParameters.timeIntervalUnit == configurationParameters.timeIntervalUnitEnum.SECOND) {
            trigger =
                TriggerBuilder.newTrigger().withDescription(configurationParameters.regionForDemo.toString()).withIdentity(configurationParameters.schedulerJobName,
                                                                                                                           configurationParameters.loadGenerationCategory.toString()).startAt(startDate).withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInSeconds((configurationParameters.timeInterval)).repeatForever()).endAt(endDate).build();
        } else if (configurationParameters.timeIntervalUnit ==
                   configurationParameters.timeIntervalUnitEnum.MILLISECOND) {
            trigger =
                TriggerBuilder.newTrigger().withDescription(configurationParameters.regionForDemo.toString()).withIdentity(configurationParameters.schedulerJobName,
                                                                                                                           configurationParameters.loadGenerationCategory.toString()).startAt(startDate).withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInMilliseconds((configurationParameters.timeInterval)).repeatForever()).endAt(endDate).build();
        }
        ...
        do {
            ...
            dateNow = new Date();
            Thread.sleep(600000);
        } while (dateNow.compareTo(endDate) < 1);
    }
}
